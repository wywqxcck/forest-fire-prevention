...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueA_F != cur->valueA_F)
    {
        GIZWITS_LOG("valueA_F Changed\n");
        ret = 1;
    }
    if(last->valueA_MQ != cur->valueA_MQ)
    {
        GIZWITS_LOG("valueA_MQ Changed\n");
        ret = 1;
    }
    if(last->valueB_F != cur->valueB_F)
    {
        GIZWITS_LOG("valueB_F Changed\n");
        ret = 1;
    }
    if(last->valueB_MQ != cur->valueB_MQ)
    {
        GIZWITS_LOG("valueB_MQ Changed\n");
        ret = 1;
    }

    if(last->valueA_temp != cur->valueA_temp)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueA_temp Changed\n");
            ret = 1;
        }
    }
    if(last->valueA_Humidity != cur->valueA_Humidity)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueA_Humidity Changed\n");
            ret = 1;
        }
    }
    if(last->valueB_temp != cur->valueB_temp)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueB_temp Changed\n");
            ret = 1;
        }
    }
    if(last->valueB_Humidity != cur->valueB_Humidity)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueB_Humidity Changed\n");
            ret = 1;
        }
    }
    if(last->valueA_CO2 != cur->valueA_CO2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueA_CO2 Changed\n");
            ret = 1;
        }
    }
    if(last->valueA_PM != cur->valueA_PM)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueA_PM Changed\n");
            ret = 1;
        }
    }
    if(last->valueB_CO2 != cur->valueB_CO2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueB_CO2 Changed\n");
            ret = 1;
        }
    }
    if(last->valueB_PM != cur->valueB_PM)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueB_PM Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(A_F_BYTEOFFSET,A_F_BITOFFSET,A_F_LEN,(uint8_t *)devStatusPtr,dataPoints->valueA_F);
    gizStandardCompressValue(A_MQ_BYTEOFFSET,A_MQ_BITOFFSET,A_MQ_LEN,(uint8_t *)devStatusPtr,dataPoints->valueA_MQ);
    gizStandardCompressValue(B_F_BYTEOFFSET,B_F_BITOFFSET,B_F_LEN,(uint8_t *)devStatusPtr,dataPoints->valueB_F);
    gizStandardCompressValue(B_MQ_BYTEOFFSET,B_MQ_BITOFFSET,B_MQ_LEN,(uint8_t *)devStatusPtr,dataPoints->valueB_MQ);
    gizByteOrderExchange((uint8_t *)devStatusPtr->rBitBuf,sizeof(devStatusPtr->rBitBuf));

    devStatusPtr->valueA_temp = gizY2X(A_temp_RATIO,  A_temp_ADDITION, dataPoints->valueA_temp); 
    devStatusPtr->valueA_Humidity = gizY2X(A_Humidity_RATIO,  A_Humidity_ADDITION, dataPoints->valueA_Humidity); 
    devStatusPtr->valueB_temp = gizY2X(B_temp_RATIO,  B_temp_ADDITION, dataPoints->valueB_temp); 
    devStatusPtr->valueB_Humidity = gizY2X(B_Humidity_RATIO,  B_Humidity_ADDITION, dataPoints->valueB_Humidity); 

    devStatusPtr->valueA_CO2 = exchangeBytes(gizY2X(A_CO2_RATIO,  A_CO2_ADDITION, dataPoints->valueA_CO2)); 
    devStatusPtr->valueA_PM = exchangeBytes(gizY2X(A_PM_RATIO,  A_PM_ADDITION, dataPoints->valueA_PM)); 
    devStatusPtr->valueB_CO2 = exchangeBytes(gizY2X(B_CO2_RATIO,  B_CO2_ADDITION, dataPoints->valueB_CO2)); 
    devStatusPtr->valueB_PM = exchangeBytes(gizY2X(B_PM_RATIO,  B_PM_ADDITION, dataPoints->valueB_PM)); 
